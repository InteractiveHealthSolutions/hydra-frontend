...
const handleResponseLogin = (response) => {
  return response.text().then((text) => {
    ...
    localStorage.setItem(
      "active_user",
      JSON.stringify(JSON.parse(text).results[0].privileges)
    );
    return "authorized";
  });
};
...
export const loginTest = (username, password) => (dispatch) => {
  var basicAuth2 = authenticationGenerator.generateAuthenticationToken(
    username,
    password
  );
  let URL =
    `http://ihs.ihsinformatics.com:9990/aahung-aagahi/api/user/username/` +
    username;
  console.log(URL);
  return axios
    .get(URL, {
