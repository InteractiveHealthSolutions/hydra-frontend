...
class Form extends React.Component {
  ...
  async componentWillReceiveProps(newProps) {
    if (newProps.getAllComponentFormRelation !== undefined) {
        console.log("componentFormMapList", newProps.getAllComponentFormRelation)
      await this.setState(
        {
          componentFormMapList:newProps.getAllComponentFormRelation
        },
        () => {
            console.log("componentFormMapList", this.state.componentFormMapList)
    ...
    if (
      newProps.formList !== undefined &&
      newProps.formList.forms !== undefined
    ) {
      let componentWithLinkedForm = await newProps.formList.forms.filter(
        data => data.component === null
      );
      await this.setState(
        {
          availableForms: componentWithLinkedForm
        },
        () => {
  ...
  async displayForms() {
    let filterList = await this.filterFormWithComponent();
    this.setState({
      listItems: filterList.map(val => (
  ...
  async handleSubmit(e) {
    ...
    await this.props.getComponentFormRelation();
    this.closeModal();
    // await this.props.searchForm(activeComponentUuid)
    // await this.props.getAllForm()
  }
  ...
  itemIncludes(element) {
    ...
    for (
      var i = 0;
      i < this.state.listItems.length && includes === false;
      i++
    ) {
      if (this.state.listItems[i].uuid === element.uuid) includes = true;
    }
    return includes;
  }
